...
void load_obj(const char *file_name, Triangle_Mesh *mesh)
{
    assert(mesh);
    ...
    std::string inputfile = std::string(file_name);
    ...
    for (size_t s = 0; s < shapes.size(); s++) {
        for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++) {
            num_vert += shapes[s].mesh.num_face_vertices[f];
        }
    }
    ...
    for (size_t s = 0; s < shapes.size(); s++) {
        ...
        for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++) {
            ...
            for (size_t v = 0; v < fv; v++) {
                ...
                auto *vert = &mesh->vertices[vert_offset + v];
                vert->x = vx;
                vert->y = vy;
                vert->z = vz;
                ...
                if (idx.normal_index >= 0) {
                    ...
                    auto *normal = &mesh->normals[vert_offset + v];
                    normal->x = nx;
                    normal->y = ny;
                    normal->z = nz;
                } else {
                ...
                mesh->uvs[vert_offset + v].x = tex_s;
                mesh->uvs[vert_offset + v].y = tex_t;
            }
